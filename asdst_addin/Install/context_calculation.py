...
from asdst_addin import the_extension
...
class ContextCalculationTool(object):
    ...
    def execute(self, parameters, messages):
        ...
        try:
            ap.Copy_management(the_extension.config.template_context_gdb, self.gdb)
        ...
        self.layer_dict = the_extension.project.layer_dictionary()
        ...
        for k, v in self.layer_dict.iteritems():
            ...
            res = [n]
            res2 = [n]
            res.extend([k, the_extension.codes[k]])  # model_code, model_desc
            res2.extend([k, the_extension.codes[k]])  # model_code, model_desc
            lyr = v["name"]
        ...
        if the_extension.config.ahims_sites:
            add_message("Analysing AHIMS points...")
            res_tot = []
            n = 0
            for k, v in the_extension.codes_ex.iteritems():
                add_message("...{0}".format(v))
                n += 1
                res = [n]
                res.extend([k, the_extension.codes_ex[k]])  # model_code, model_desc

                tmp_fc = join(self.gdb, "ahims_{0}_context".format(k))
                if the_extension.config.ahims_sites:
                    ap.Intersect_analysis([the_extension.config.ahims_sites, self.context], tmp_fc)
                ...
                if the_extension.config.ahims_sites:
                    ap.Intersect_analysis([the_extension.config.ahims_sites, self.assessment], tmp_fc)
                    sc = ap.da.SearchCursor(tmp_fc, "*", '"{0}" IS NOT NULL'.format(k))
                    l = [r for r in sc]
                    res.append(len(l))  # context_pts
                else:
                ...
                if the_extension.config.ahims_sites:
                    ap.Intersect_analysis([the_extension.config.ahims_sites, self.conservation], tmp_fc)
                    sc = ap.da.SearchCursor(tmp_fc, "*", '"{0}" IS NOT NULL'.format(k))
                    l = [r for r in sc]
        ...
        add_message(the_extension.compact_fgdb(self.gdb))  # note the side effect

        # Add data to map
        add_message("Adding feature layers to map...")
        lyrs = {k: v[0] for k, v in self.areas.iteritems()}
        the_extension.project.add_layers(lyrs, "Context {}".format(self.sane_title), "calc")
        add_message("Adding result tables to map...")
        the_extension.add_table(the_extension.project.mxd, self.table_summ, "context_loss")
        the_extension.add_table(the_extension.project.mxd, self.table_ahims, "context_ahims")

        # Save and report status
        the_extension.project.mxd.save()
