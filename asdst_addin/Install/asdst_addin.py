...
class CreateProjectButton(object):
    ...
    def onClick(self):
        ...
        try:
            if config.system_config_is_valid() and config.user_config_is_valid():
...
class BuildDataButton(object):
    ...
    def onClick(self):
        ...
        try:
            if config.system_config_is_valid() and config.user_config_is_valid() and config.map_config_is_valid():
...
def _enable_tools(set_all_to_false=False):
    print "_enable_tools(set_all_to_false={})".format(set_all_to_false)

    if set_all_to_false:

        ConfigureButton.enabled = False
        CreateProjectButton.enabled = False
        BuildDataButton.enabled = False
    ...
    CreateProjectButton.enabled = config.system_config_is_valid() and config.user_config_is_valid()

    BuildDataButton.enabled = config.map_config_is_valid()

    CalculateContextButton.enabled = config.asdst_data_is_valid()
...
#
#     config_status = [utils.exists_return_3tuple(k, v) for k, v in config.iteritems()]
#
#     if not pretty:
